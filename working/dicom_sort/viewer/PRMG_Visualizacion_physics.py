"""
DICOM sequence reviewer — physics reference overlay.

Identical to PRMG_Visualizacion_local.py except:
  - Main CSV  (--excel) : DCM-classifier output  (same as original)
  - Reference (--ref)   : physics-label CSV from label_physics.py

The reference overlay shows the physics label next to each image and is
colour-coded against the active classifier filter:
  green  – physics agrees with the current DCM-clf filter
  red    – physics disagrees
  grey   – no physics data available
"""
...
# ---------------------------------------------------------------------------
# Reference-label helpers  (physics side)
# ---------------------------------------------------------------------------

def _phys_fat_yn(val) -> str:
    """FS or STIR → Y, empty → N."""
...
def ref_label_color(row, modality: str, fatsat: str, contrast: str) -> str:
    """Green if physics label agrees with the active classifier filter, red otherwise."""
    seq = row.get("phys_sequence", "")
    fs  = row.get("phys_fat_sat",  "")
    ce  = row.get("phys_contrast", "")
    if pd.isna(seq) or str(seq).strip() == "":
        return "#888888"
    phys_fs = _phys_fat_yn(fs)
    phys_ce = _phys_contrast_yn(ce)
    if str(seq).strip() == modality and phys_fs == fatsat and phys_ce == contrast:
        return "#44dd44"
    return "#ff4444"


# ---------------------------------------------------------------------------
# DICOM loading  (unchanged from PRMG_Visualizacion_local.py)
# ---------------------------------------------------------------------------
...
def pad_image(img, target_height, target_width):
    ...
    return np.pad(img,
                  ...
                  mode="constant", constant_values=0)[:target_height, :target_width]


# ---------------------------------------------------------------------------
# Batch navigation  (unchanged)
# ---------------------------------------------------------------------------
...
def update_view():
    ...
    for i, (_, row) in enumerate(batch.iterrows()):
        dcm_path = os.path.normpath(
            str(row["Nombre DICOM"]).replace("/Project", r"Z:\mnt\rimp\PROJECTS\BONE-AI")
        )
        ...
        try:
            ...
            # Physics reference label (colour-coded agreement)
            if ref_csv_path:
...
# ---------------------------------------------------------------------------
# CLI  (same flags as original; --ref now points to physics CSV)
# ---------------------------------------------------------------------------
...
_parser.add_argument("--contrast", default="N", choices=["Y", "N"],
                     help="Contrast-enhanced filter Y/N (default: N)")
_parser.add_argument("--excel",
                     ...
                     help="Path to DCM-classifier output CSV")
...
# Filter columns — same as original viewer (DCM classifier)
...
output_excel      = os.path.join(output_folder, f"Resultados_revision_{sequence_name}.csv")
output_traz_dir   = os.path.join(output_folder, "Backup")
os.makedirs(output_traz_dir, exist_ok=True)
output_excel_traz = os.path.join(output_traz_dir, f"Review_{sequence_name}_{timestamp}.csv")
...
# ---------------------------------------------------------------------------
# Load and filter  (same as original — filter on classifier columns)
# ---------------------------------------------------------------------------
...
for index, col in enumerate(secuencia_colum):
    df_filtered = df_filtered[
        df_filtered[col] == sequence_filter[index]
    ].reset_index(drop=True)

# Merge optional physics reference
_phys_cols = ["phys_sequence", "phys_acquisition", "phys_fat_sat", "phys_contrast"]
df_filtered = df_filtered.drop(columns=[c for c in _phys_cols if c in df_filtered.columns])
...
if ref_csv_path:
    ...
    df_filtered = df_filtered.merge(
        df_ref[["Paciente", "Estudio", "Serie"] + _phys_cols],
        on=["Paciente", "Estudio", "Serie"], how="left"
    )

# Resume from last unreviewed page
...
root = tk.Tk()
root.title(f"DICOM reviewer — {sequence_name}  [physics ref]")
