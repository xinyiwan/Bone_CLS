"""
Extract 2D VLM crops directly from the segmentation project's real on-disk
...
Discovery is segmentation-driven via pairs.find_pairs, so only scans that have a
mask are considered (a reviewed mask wins over a history one).
...
Usage:
  ...
  python extract_from_dataset.py --data-root /data \\
      --sequence-table sequences.csv --config feature_config_dataset.yaml \\
      --out-root ./out --index-only

  # 2. Extract:
  python extract_from_dataset.py --data-root /data \\
      --sequence-table sequences.csv --config feature_config_dataset.yaml \\
      --out-root ./out --overlay
"""
...
def build_index(
    ...
    rows: List[Record] = []
    n_seen = n_no_label = n_no_image = 0
...
def main() -> None:
    ...
    # sequence-table column names (override to match your file; e.g. this
    # project's table names the subject column 'case' -> --seq-subject-col case)
    ...
    # pipeline options (mirror run.py)
